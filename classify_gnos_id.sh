...
do
  ...
  do	
	comm -1 -2 <(sort repos/$r/$r.live.gnos_id.txt | uniq) <(sort indexed_gnos_id/$t.gnos_id.txt | uniq) | sort -u > repos/$r/$r.live.index.$t.gnos_id.txt
	comm -1 -2 <(sort repos/$r/$r.live.index.$t.gnos_id.txt | uniq) <(sort gnos_id_on_portal.txt | uniq) | sort -u > repos/$r/$r.live.index.$t.portal.gnos_id.txt
	comm -2 -3 <(sort repos/$r/$r.live.index.$t.gnos_id.txt | uniq) <(sort gnos_id_on_portal.txt | uniq) | sort -u > repos/$r/$r.live.index.$t.not_portal.gnos_id.txt
	...
	less repos/$r/$r.live.index.$t.gnos_id.txt >> repos/$r/$r.live.index.gnos_id.txt
  done
done
